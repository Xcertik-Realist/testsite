...
export default function CheckoutPage() {
  ...
  // Perfectly typed delivery prices
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="space-y-5">
              ...
              <textarea placeholder="Delivery Address" required rows={3} value={formData.address}
              ...
              <div>
                <input type="text" placeholder="Postcode (e.g. SW1A 1AA)" required value={formData.postcode}
                  onChange={(e) => setFormData({ ...formData, postcode: e.target.value.toUpperCase() })}
                  className={`w-full px-5 py-4 rounded-xl border focus:outline-none text-lg ${
                    isValidPostcode === false ? "border-red-500" : isValidPostcode ? "border-green-500" : "border-gray-300 focus:border-green-700"
                  }`} />
              ...
              <select value={formData.deliveryOption}
                onChange={(e) => setFormData({ ...formData, deliveryOption: e.target.value as any })}
                className="w-full px-5 py-4 rounded-xl border border-gray-300 focus:border-green-700 focus:outline-none text-lg">
                <option value="standard">Standard (£9.99) – 3–5 days</option>
                <option value="express">Express (£14.99) – Next day</option>
                <option value="chooseday">Choose Day (£19.99) – Pick date</option>
              </select>

              <button
                type="submit"
                disabled={isSubmitting || !isValidPostcode}
